...
type TableProps<T> = {
  ...
  getEditLink: string | ((item: T) => string);
};
        ...
        <thead className="bg-gray-800 text-gray-300">
          <tr>
            {columns.map((col) => (
              <th
                key={col.key}
                className="px-4 py-2 text-center font-normal text-sm cursor-pointer"
            ...
            {(onEdit || onDelete) && (
              <th className="px-4 py-2 text-center font-normal text-sm">Actions</th>
            )}
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-700">
          {sortedData.map((row, idx) => (
            <tr key={idx} className="hover:bg-gray-700">
              {columns.map((col) => (
                <td key={col.key} className="px-4 py-3 text-center whitespace-nowrap">
              ...
              {(onEdit || onDelete) && (
                <td className="py-3 text-center whitespace-nowrap">
                  <div className="flex justify-center space-x-2">
                    {onEdit && (
                      <Link to={typeof getEditLink === 'function' ? getEditLink(row) : getEditLink}>
                        <button className="text-blue-400 hover:text-blue-600">Show</button>
                      </Link>
                    )}
                    {onDelete && (
                      <button
